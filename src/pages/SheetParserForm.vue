...
export default defineComponent({
  ...
  data() {
    return {
      sheet: {
        id:'1Tud14meApBZzcoMgD9vyx2LaVl1fRqhknrgygIGWWaM',
        titol: 'Nota',
        numHeaders: 2,
        numRowsAlumnes: 2,
        numColumnEmail: 3
  ...
  methods: {
    ...
    draftEmail: async function(){
      ...
      this.emails = [];
      this.emailsAlumne = [];
      const liniesFetch = await axios.post(process.env.API + '/api/sheetparser/draft',this.sheet.id)
    ...
    sendEmail: async function(){
      ...
      await axios.post(process.env.API + '/api/sheetparser/send',{
